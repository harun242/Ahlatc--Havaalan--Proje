...
namespace AırportMvcUI.ApiServices
{
    public class HttpApiService : IHttpApiService
    {
        ...
        public async Task<bool> DeleteData(string requestUri)
        {
            var requestMessage = new HttpRequestMessage()
            {
                Method = HttpMethod.Delete,
                RequestUri = new Uri($"http://localhost:5044/api{requestUri}"),
                Headers =
        {
		...
		public async Task<T> PutData<T>(string requestUri, string jsonData)
		{
			T response = default(T);
